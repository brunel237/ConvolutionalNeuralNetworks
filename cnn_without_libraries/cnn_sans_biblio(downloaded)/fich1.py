...
def fully_connected(pooled):
    (nf2, dim2, _) = pooled.shape
    fc = pooled.reshape((nf2 * dim2 * dim2, 1)) # flatten pooled layer
    return fc

def dense_layer(w3,w4,b3,b4,fc):
...
#####################################################
import os
def train(num_classes = 2, lr = 0.01, beta1 = 0.95, beta2 = 0.99, img_dim = 175, img_depth = 1, f = 5, num_filt1 = 8, num_filt2 = 8, batch_size = 32, num_epochs = 2, save_path = 'params.pkl'):

    # Get training data
    # base_dir = '/media/brunel/ubuntu_xtend2/Devoir242/myCNN/img_data'
    # cat_names = []
    # dog_names = os.listdir(base_dir+'/dog')
    # for c in os.listdir(base_dir+'/cat'):
    #     cat_names.append(float(c))
    # for c in os.listdir(base_dir+'/dog'):
    #     dog_names.append(float(c))
    
    m =50000
    X = extract_data('/media/brunel/ubuntu_xtend2/Devoir242/myCNN/training_set.zip', m, img_dim)
    y_dash = extract_labels('/media/brunel/ubuntu_xtend2/Devoir242/myCNN/training_set.zip', m).reshape(m,1)
    # X = (cat_names)+(dog_names)
    # y_dash = ['cat', 'dog']
    X-= int(np.mean(X))
    X/= int(np.std(X))
